...
## 2. Estructura de Capas Obligato### A. Capa de Controladores (Controllers)
...
- **Retorno:** DEBEN devolver siempre `ResponseEntity<ApiResponse<T>>`. El `ApiResponse` se construye y se envuelve en esta capa (Controller) y no en la capa de servicios.
...
- **Resolución de Contexto:** Si se requiere información del usuario/desarrollo (`UserContext`), se debe obtener en el Controller y pasarse como argumento al Service.

### B. Capa de Servicios (Services)
- **Anotaciones:** Usa `@Service`, `@Slf4j` (si requiere logs técnicos) y `@RequiredArgsConstructor`.
- **Responsabilidad:** 1. Orquestar llamadas al `Repository`.
    2. Disparar integraciones (ej. `NotificationClient`, `StorageClient`).
    3. Registrar auditoría de negocio usando `BusinessLogger`.
- **Retorno:** DEBEN devolver el tipo de dato puro (ej. `List<T>`, `Boolean`, DTOs) o lanzar excepciones de negocio. No construyas ni devuelvas `ApiResponse` en esta capa.
- **Regla de Contexto:** No inyectes `UserContext` en la capa de servicios ni en repositorios. Todo valor contextual (ej. `idDesarrollo`, `usuario`) debe recibirse como parámetro desde el Controller.
...
- **Lógica Matemática/Negocio en Java:** Prohibido calcular impuestos, validaciones de stock o penalizaciones en Java. Llama a la función o SP de SQL Server para que te devuelva el valor.
- **Romper el ApiResponse:** Prohibido devolver un `String`, un `List<T>` crudo o un `Map` directamente en el Controller. TODO sale envuelto en `ApiResponse<T>`.
- **Cadenas quemadas (Hardcoding):** Prohibido usar roles quemados en `@PreAuthorize` si existe un Enum de permisos. Prohibido quemar URLs de otros servicios (usa `@Value`).
- **Pasar el Request a la BD:** Prohibido pasar el objeto HTTP Request/Response o sesiones directamente al Repository. El Service extrae los datos puros y los manda.
...
public class EjemploController {
    ...
    public ResponseEntity<ApiResponse<Boolean>> procesar(@Valid @RequestBody EjemploRequest request) {
        ...
        boolean resultado = service.procesarDatos(request, usuario);
        return ResponseEntity.ok(ApiResponse.success("Proceso exitoso", resultado));
    }
}
...
public class EjemploService {
    ...
    public boolean procesarDatos(EjemploRequest request, String usuario) {
